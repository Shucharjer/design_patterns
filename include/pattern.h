...
namespace pattern {
    ...
    namespace simplefactory {
        ...
        public:
            ...
            [[nodiscard]] std::shared_ptr<SpecificProductType> Create(Args&&... args) { return std::make_shared<SpecificProductType>(std::forward<Args>(args)...); }
        protected:
    ...
    namespace proxy {
        ...
        public:
            ...
            Proxy(std::function<std::unique_ptr<T>()> object_create_method) : object_create_method_(object_create_method), proxied_object_(nullptr) {}
    ...
    class Handler {
    public: 
        virtual void SetNextHandler(Handler<T>& next) {
            next_ = &next;
        }
    protected:
        Handler<T>* next_;
        ...
        public:
            ...
            auto& Get(InternalState_&& internal_state) {
                if (pool_.find(internal_state) == pool_.end()) {
                    pool_[internal_state] = std::make_shared<ConcreteState>(std::forward<InternalState>(internal_state));
                }
